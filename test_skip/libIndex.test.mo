    ...
    func() {
        ...
        test(
            ...
            func() {
                ...
                libIndex.insert_many_at_index(index0, 1, [2, 3, 4, 5]);
                assert libIndex.get_last_index(index0) == null;

                libIndex.insert_many_at_index(index0, 0, [3]);
                assert libIndex.get_wrapped_blob_Address(index0, 0) == ?3;

                libIndex.insert_many_at_index(index0, 0, [2]);
                assert libIndex.get_wrapped_blob_Address(index0, 0) == ?2;

                libIndex.remove_at_index(index0, 0);
                libIndex.remove_at_index(index0, 0);

                assert libIndex.get_wrapped_blob_Address(index0, 0) == null;
                assert libIndex.get_wrapped_blob_Address(index0, 1) == null;

                libIndex.insert_many_at_index(index0, 0, [2, 3, 4, 5]);
                assert libIndex.get_last_index(index0) == ?3;

                assert libIndex.get_wrapped_blob_Address(index0, 0) == ?2;
                assert libIndex.get_wrapped_blob_Address(index0, 1) == ?3;
                assert libIndex.get_wrapped_blob_Address(index0, 2) == ?4;
                assert libIndex.get_wrapped_blob_Address(index0, 3) == ?5;
                assert libIndex.get_wrapped_blob_Address(index0, 4) == null;

                libIndex.insert_many_at_index(index0, 2, [23, 27]);

                assert libIndex.get_wrapped_blob_Address(index0, 0) == ?2;
                assert libIndex.get_wrapped_blob_Address(index0, 1) == ?3;
                assert libIndex.get_wrapped_blob_Address(index0, 2) == ?23;
                assert libIndex.get_wrapped_blob_Address(index0, 3) == ?27;
                assert libIndex.get_wrapped_blob_Address(index0, 4) == ?4;
                assert libIndex.get_wrapped_blob_Address(index0, 5) == ?5;
                assert libIndex.get_wrapped_blob_Address(index0, 6) == null;
