...
module {
    public func init() : Bench.Bench {
        ...
        bench.name("Remove at index");
        bench.description("Remove at index - benchmark. (slow)");

        bench.rows(["Remove value at first index","Remove value at middle index","Remove value at end index"]);
        ...
        bench.runner(
            func(row, col) {
                ...
                switch (row) {
                    case ("Remove value at first index") {
                        rowNumberText := "1";
                    };
                    case ("Remove value at middle index") {
                            rowNumberText := "2";
                    };
                    case ("Remove value at end index") {
                        rowNumberText := "3";
                    };
                    case (_) {
                        // do nothing
                ...
                switch (row) {
                    case ("Remove value at first index") {
                        for (i in Iter.range(1, n)) {
                            ...
                            ignore hashList.remove_at_index(key, 0);
                        };
                    };
                    case ("Remove value at middle index") {
                        for (i in Iter.range(1, n)) {
                            ...
                            let index : Nat = (n - i) / 2;
                            ignore hashList.remove_at_index(key, index);
                        };
                    };
                    case ("Remove value at end index") {
